...
\subsubsection{Axis Discovery Through Residual Analysis}

When the model fails to score a shot type, the residuals contain information about what is missing. Rather than accepting the failure or manually proposing new axes, we ask: what direction in the encoder's 1024-dimensional feature space separates the shots the model scores well from those it scores badly?

We identify five problem categories with transfer MAE above 2.5 (smash, forehand slice, forehand volley, slice service, backhand two-handed). We compute the mean feature vector of the problem group and the mean of the well-scored group, take the difference, normalise it, and project all shot types onto this direction. This projection becomes the score for a 7th axis, scaled to 0--10.

\begin{table}[ht]
\centering
\caption{Before vs.\ after adding a data-derived 7th axis to the tennis domain}
\begin{tabular}{lrrrl}
\toprule
Shot type & 6-axis MAE & 7-axis MAE & Change & Verdict \\
\midrule
forehand slice & 3.30 & 1.89 & $-$1.42 & \textbf{IMPROVED} \\
kick service & 1.91 & 1.16 & $-$0.74 & \textbf{IMPROVED} \\
backhand 2H & 2.52 & 1.84 & $-$0.68 & \textbf{IMPROVED} \\
forehand volley & 2.74 & 2.12 & $-$0.63 & \textbf{IMPROVED} \\
forehand open & 1.93 & 1.42 & $-$0.51 & \textbf{IMPROVED} \\
forehand flat & 1.67 & 1.30 & $-$0.37 & \textbf{IMPROVED} \\
slice service & 3.46 & 3.22 & $-$0.23 & \textbf{IMPROVED} \\
flat service & 1.33 & 1.17 & $-$0.17 & \textbf{IMPROVED} \\
backhand slice & 1.68 & 1.87 & $+$0.18 & regressed \\
smash & 3.14 & 3.62 & $+$0.48 & regressed \\
backhand & 1.29 & 1.98 & $+$0.69 & regressed \\
backhand volley & 0.82 & 1.65 & $+$0.83 & regressed \\
\midrule
\textbf{Mean} & \textbf{2.15} & \textbf{1.94} & $-$\textbf{0.21} & \\
\bottomrule
\end{tabular}
\label{tab:axis_discovery}
\end{table}

The 7th axis improves 8 of 12 shot types and reduces overall transfer MAE from 2.15 to 1.94. The four regressions are mild (largest: $+$0.83) while the improvements are substantial (largest: $-$1.42). This is not overfitting: an overfit axis would improve the problem categories at the cost of destroying the well-scored ones. The improvement is broadly distributed.

Examining which shot types score highest on the discovered axis reveals its meaning. Forehand volley (6.5), smash (6.4), slice service (6.3), and forehand slice (6.0) score high. Backhand (2.5) and backhand slice (3.7) score low. The axis separates a forward-and-down cutting motion through the ball from a horizontal flat swing. A coach might call this ``angle of attack'' or ``contact plane.'' The data found it; the human names it after the fact.

This closes a loop in the framework. The system scores videos, identifies where it fails, locates the missing dimension in feature space, proposes a new axis, and demonstrates improvement on held-out transfer. The axes evolve with the data. The human decides whether to accept the proposal. The model identifies the gap; the researcher fills it.

Smash remains poorly scored (3.14 $\to$ 3.62) because its overhead dynamics require more than one additional axis to capture. The residual analysis could be applied iteratively: after adding axis 7, recompute residuals, discover axis 8. Each iteration captures the next most informative dimension the model is missing. We leave this iterative extension to future work.
...
\subsubsection{Beyond Unit Precision: Normal-Gamma Extension}

The Gaussian formulation above hardcodes the observation precision to a single value, so the posterior precision grows linearly with $n$ regardless of how scattered the observations actually are. The second-moment information ($\sum x_i^2$) is never used. This is fine when valence axes are pre-normalised to unit variance, but production valence vectors carry per-axis scales that differ by orders of magnitude, and the unit-precision model has no mechanism to discover them.

We provide a Normal-Gamma alternative \citep{bishop2006} that closes this gap. Each axis carries four hyperparameters $(\mu_n, \beta_n, a_n, b_n)$ representing the joint posterior over $(\mu, \tau)$ --- mean and precision both treated as random variables:
...
\textbf{Effect on memory composition.} Re-running the firefighter scenario (77 observations, capacity 20, identical RNG, identical thresholds) with the Normal-Gamma posterior keeps \textbf{all 5 of 5 EVACUATE-class events} (3 crises + 2 backdrafts) versus \textbf{1 of 5} under the original Gaussian. The mean is learned at the same conjugate rate in both, but the Normal-Gamma additionally learns per-axis variance, so high-intensity events on the noisy `intens' axis ($\sigma \approx 2.0$) are correctly tagged as several-$\sigma$ events with high LOO influence and survive eviction. The unit-precision Gaussian has no scale concept and ranks routine variation alongside genuine crises.

The Normal-Gamma posterior is available as \texttt{BayesianMemory(posterior\_kind="normal\_gamma")}. The Gaussian remains the default for backward compatibility.
...
\textbf{End-to-end JEPA world models.} Concurrent work by \citet{maes2026} introduces LeWorldModel, a JEPA that trains stably end-to-end from raw pixels with only 15M parameters on a single GPU, using just two loss components (next-embedding prediction and Gaussian regularisation). Their learned latent space encodes physical quantities and supports surprise-based detection of implausible events, achieving planning 48$\times$ faster than foundation-model-based alternatives. This is directly relevant to vScore in two ways. First, it offers a path to replacing the frozen V-JEPA~2 encoder (300M parameters, pretrained) with a lightweight, domain-specific encoder trainable from scratch on target videos. Second, their native surprise signal in latent space complements vScore's Bayesian memory, though LeWorldModel provides surprise detection without the selective retention, posterior-aware eviction, coverage maintenance, and periodic replay that constitute vScore's memory management system. The two approaches are complementary: LeWorldModel provides the encoder and the surprise; vScore provides the scoring, the action inference, and the memory.
...
\subsection{Coherence Scoring: When Disorder Is the Signal}

The current framework assumes that every action has a coherent valence profile: a direction in valence space that characterises it. But every domain contains actions defined by their \emph{incoherence}. In tennis, a frame shot (``bois'') occurs when a player is overwhelmed by speed and produces an unplanned, improvised response. In hockey, a scramble in front of the net involves bodies and sticks moving without structured intent. In firefighting, a collapse event produces chaotic motion with no coherent trajectory.

These actions are rare but real. They are not noise; they are a legitimate category whose signature is \emph{dispersion} rather than direction. A forehand flat has low feature variance across examples because every player executes roughly the same motion. A frame shot has high feature variance because each one is different in a different way.

The current random baseline cannot distinguish between ``no signal'' (the axis genuinely carries no information) and ``the signal is disorder'' (the action is genuinely unstructured). Both produce transfer MAE near the random level, but for opposite reasons. A second test is needed: compare the encoder feature variance of the candidate category against structured categories. High transfer MAE combined with high feature variance indicates genuinely unstructured motion, not a failure of the scoring framework.

This suggests an additional output alongside the valence vector: a \emph{coherence score} measuring how tightly the current observation clusters with known patterns. A coherent shot has low feature variance and a clear valence direction. A scramble has high variance and no consistent direction. The coherence score would function as a meta-axis: not ``what kind of action is this'' but ``how much does this resemble a structured action at all.'' This applies across all sports and all domains where improvised, desperate, or chaotic responses occur alongside trained, structured ones.
...
\newblock Springer. \S2.3.6, conjugate Normal-Gamma prior for univariate Gaussian with unknown mean and precision.
...
\bibitem[Maes et~al.(2026)]{maes2026}
